...
func handleRequests(){
	 ...
	 */
	myRouter.HandleFunc("/devices",financierServer).Methods("POST")
    myRouter.HandleFunc("/notify_paid",financierServer).Methods("POST")
	myRouter.HandleFunc("/credit_completed",financierServer).Methods("POST")
...
func  financierServer(w http.ResponseWriter,r *http.Request) {
    ...
    r.ParseForm()
    origin_path:=r.RequestURI
    path:=strings.Trim(origin_path,"/")
    fmt.Println(path)

	body:=r.Form
	for k,v:=range body{
		fmt.Printf("k is %s, type is %s\n",k,reflect.TypeOf(k))
		fmt.Printf("v is %s, type is %s\n",v,reflect.TypeOf(k))	}
	fmt.Println("body is ",body)
	tag:=body["tag"][0]
	curef:=body["curef"][0]

	api:=strings.Split(tag,"-")[1]
	newMap:=make(map[string]string,0)
	fmt.Println("tag is ",api)
  ...
  //  imei:=r.FormValue("imei")
   // tag:=r.FormValue("tag")
	...
	case "ping":
		//ff.Ping(imei,tag)
	case "devices":
		curef:=r.FormValue("curef")
		imei:=r.FormValue("imei")
		if len(imei)==0{
			fmt.Println("no imei")
		}
        ff.Devices(w,curef,imei,tag,now)
	...
	case "notify_paid":
		...
		ff.Notify_paid(w,curef,tag,newMap,now)

	case "credit_completed":
		//msg:=r.FormValue("msg")
		//ff.Notify_credit_completed(imei,tag,msg)
		for k,v:=range(body){
			if k!="tag" && k!="curef"{
				newMap[k]=v[0]
			}
		...
		fmt.Println("newmap is ",newMap)
		ff.Notify_credit_completed(w,curef,tag,newMap,now)
