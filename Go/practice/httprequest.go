...
func handleRequests(){
	...
	myRouter.HandleFunc("/devices/{imei}",financierServer).Methods("GET")
	myRouter.HandleFunc("/devices/{imei}/notify_paid",financierServer).Methods("POST")
	myRouter.HandleFunc("/devices/{imei}/notify_credit_completed",financierServer).Methods("POST")
...
func portaltool(w http.ResponseWriter, r *http.Request) {
	...
	case "DeviceFinancier":
		...
		if (strings.Index(imeis, ",") == -1){
			panic("Imeis must split by comma -> aaa,bbb,ccc")
		}
...
func  financierServer(w http.ResponseWriter,r *http.Request) {
	var fs financier.FinancierServer
	var ff financier.FinancierFunction = fs

	routeVars:=mux.Vars(r)
	fmt.Println("routevars is ",routeVars)
	r.ParseForm()

	path:=r.RequestURI
    deviceImei:= "/devices/" + routeVars["imei"]
    //notifyImei:= "device/" + routeVars["imei"] + "/notify_paid"
    //completeImei:= "device/" + routeVars["imei"] + "/notify_credit_completed"

    fmt.Println("deviceImei: is",deviceImei)
	...
	case "ping":
		...
		ff.Ping(imei)
	case deviceImei:
		fmt.Println("hit device/{imei}")
		imei:=r.FormValue("imei")
		ff.Devices(imei)
	case "notify_paid":
		fmt.Println("hit notify_paid")
		imei:=r.FormValue("imei")
		nextPayDl_string:=r.FormValue("nextPayDl")
		if nextPayDl,err:= strconv.Atoi(nextPayDl_string);err!=nil {
			ff.Notify_paid(imei, nextPayDl)
		} else {
			fmt.Println("strconv.Atoi error",err)
		}
	case "notify_credit_completed":
		fmt.Println("hit notify_credit_completed")
		imei:=r.FormValue("imei")
		msg:=r.FormValue("msg")
		ff.Notify_credit_completed(imei,msg)
	}
