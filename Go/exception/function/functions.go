...
import (
	...
	"display_portal/exception/cassandra"
	"encoding/json"
	"fmt"
	"os/exec"
	"strconv"
...
loop:
	...
	if err := cmd.Run(); err != nil {
		fmt.Printf( "Run Bash script %s failed: %s : %s,sleep 30 seconds and rollback and run again(%d)\n", command, fmt.Sprint( err ), stdErr.String(),count )
		time.Sleep(100)
		cmderr := "/data/bin/src/exception/cmd.out"
		WriteFile(cmderr)
		goto loop
	}else {
...
func WriteFile(f string,c interface{}){
	tz := "Asia/Shanghai"
	location,_:=time.LoadLocation(tz)
	time_now:=time.Now().In(location).Format("2006-01-02 15:04:05")
	fo,err:=os.OpenFile(f, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0666)
...
func TraceFile(ch chan string,file string, wg *sync.WaitGroup){
	...
	for {
		...
		savetime:="/data/bin/src/exception/time.txt"
		...
		for layer_env, iplist := range nodelist {
			...
			for _, ip := range strings.Split(iplist,",") {
				...
				fmt.Printf("Get Stat from %v - %v -%v -%v -%v -%v\n",env,ip,layer,mtime_stamp,mtime_last,size)
				...
				if mtime_stamp > mtime_last && size != "0 byte" {
					...
					fmt.Printf("Read File from %v - %v -%v\n",env,ip,layer)
