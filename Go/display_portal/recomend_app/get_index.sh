...
ssh ${user}@${gk} "ssh ${cass} \"sudo su - cassadmin -c \\\"$CQLSH  -e \\\\\\\"select name from ${KEYSPACE}.app_summary where id='${app_id}';\\\\\\\" \\\"\"" >${tmp_dir}/out.log
...
echo -n "APP_2 table:"
ssh ${user}@${gk} "ssh ${cass} \"sudo su - cassadmin -c \\\"$CQLSH  -e \\\\\\\"PAGING  OFF; select id,display,version, recommended_index from ${KEYSPACE}.app_2 where id='${app_id}';\\\\\\\" \\\"\""|sed 's/ //g;1d;4d;$d' |tee ${tmp_dir}/app_2.log


# Get from app_version table
echo -n "APP_Version table:"
ssh ${user}@${gk} "ssh ${cass} \"sudo su - cassadmin -c \\\"$CQLSH  -e \\\\\\\"PAGING  OFF;select app_id,version,supported_platforms,version_status,recommended_index from ${KEYSPACE}.app_version where app_id='${app_id}';\\\\\\\" \\\"\"" |sed 's/ //g;1d;4d;$d'|tee ${tmp_dir}/version.log
...
echo "=========Search ALL APPS=========="

echo "APP_2 table (only show the app set index > 0):"
ssh ${user}@${gk} "ssh ${cass} \"sudo su - cassadmin -c \\\"$CQLSH  -e \\\\\\\"PAGING  OFF;select id,display,version, recommended_index from ${KEYSPACE}.app_2;\\\\\\\" \\\"\"" |sed 's/ //g;1d;4d;$d'|awk -F '|' '{if ($4 != 0 && $4 != "null") print $0}'|tee ${tmp_dir}/app_2.log
...
ssh ${user}@${gk} "ssh ${cass} \"sudo su - cassadmin -c \\\"$CQLSH  -e \\\\\\\"PAGING  OFF;select app_id,version,supported_platforms,version_status,recommended_index from ${KEYSPACE}.app_version;\\\\\\\" \\\"\""|sed 's/ //g;1d;4d;$d' |awk -F '|' '{if ($5 != 0 && $5 != "null") print $0}' |tee ${tmp_dir}/app_2.log
