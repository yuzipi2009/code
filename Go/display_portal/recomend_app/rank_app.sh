...
# Making sure the application with the given id exists in app_summary!
...
#out_2log is for app_2, out.log is for app_version
...
do
    ...
    query="update ${KEYSPACE}.app_2 set recommended_index=$recommended_index where id='${app_id}' and version='${version}';"
	echo "[DEBUG] $query"
	ssh ${user}@${gk} "ssh ${cass} \"sudo su - cassadmin -c \\\"$CQLSH  -e \\\\\\\"${query}\\\\\\\" \\\"\""
done
#VVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV
