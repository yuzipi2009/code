...
mv csv_dir/curef.csv csv_dir/curef.csv_${today}
...
# Copy to GK5
[[ $? -eq 0 ]] && ssh ${user}@${prod_gk} "rm -rf /tmp/app_version.csv /tmp/app_summary.csv /tmp/curef.csv ; scp ${prod_cass}:/tmp/app_version.csv /tmp && scp ${prod_cass}:/tmp/app_summary.csv /tmp && scp ${prod_cass}:/tmp/curef.csv /tmp" || { echo "Copy app_version or app_summary or curef from cassandra failed, Aborting";exit;}

# Copy to local
[[ $? -eq 0 ]] && rm -rf app_version.csv  app_summary.csv  curef.csv; scp ${user}@${prod_gk}:/tmp/app_version.csv ./ && echo "Fetch app_version.csv to local.. OK" && scp ${user}@${prod_gk}:/tmp/app_summary.csv  ./ && echo "Fetch app_summary.csv to local.. OK" && scp ${user}@${prod_gk}:/tmp/app_curef.csv  ./  && echo "Fetch curef.csv to local.. OK"|| { echo "Copy app_version or app_summary or curef from K5-GK failed, Aborting";exit;}

# cut head and remove space
sed -i '1d' app_version.csv && sed -i 's/\s\+//g' app_version.csv

sed -i 's/\s\+//g' curef.csv
#sed -i '1d' ${abs_dir}/app_summary.csv && sed -i 's/\s\+//g' ${abs_dir}/app_summary.csv
!
#=============================================================================
# New extension: whitelist, below loop is to get 2 maps and a file for curef
echo > whitelist/full_cu_id_list.txt
echo > whitelist/white_module_${app_id}.txt
        ...
        if [ "${line_2}x" == "x" ];then
            ...
            if [ "${cu_col}x" != "x" ];then
                ...
                if [ "${black_list}" != "null" ];then
                    ...
                    # format white_module_2_${app_id}.txt to adjust ajax json
                    count=0
...
#scp data.json kai-user@10.81.74.17:./
#ssh kai-user@10.81.74.17 "sudo su -c ' cd ${ajax_dir} &&  mv data.json data.json_${today} &&  mv /home/kai-user/data.json ${ajax_dir}/'"
#[ $? -eq 0 ] && echo "Copied to Django server"

# Replace the Today and NUMBER key work in table.html
#ssh kai-user@10.81.74.17 "sudo sed -i 's/${old_date}/${today}/;s/${old_num}/${num}/g' ${table}"
